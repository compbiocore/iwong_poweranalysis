...
![](https://i.imgur.com/sBr8GBr.png)

## Parsing DataMat

```{r, file_parsing, warning=FALSE, tidy=TRUE}
# file parsing and cleaning
d <- read.delim("/home/rstudio/iwong_poweranalysis/metadata/GSE184156_RNAseq_ECM_rnaseq_featurecounts.txt", header=TRUE, sep="\t", skip=1)
...
![](https://i.imgur.com/pyNUpEV.png)

#### Extracting all genes from GeneCards where the gene-name contains either 'cadherin' or 'integrin' (93 genes)


![](https://i.imgur.com/API1sJl.png)

## Parsing Gene IDs
```{r}
genelist <- read.delim("/home/rstudio/iwong_poweranalysis/metadata/cadherin_and_integrin.txt", header=TRUE, sep="\t")
...
power2 <- est_power_distribution(n=6,f=0.01,rho=2,distributionObject=distribution,selectedGenes=genelist$Gene.name,storeProcess = TRUE)
mean(power2$power)
```
...
![](https://i.imgur.com/AqtTpAX.png)
![](https://i.imgur.com/88BXxzc.png)
...
coverage5_fdr1 <- est_power_curve(n=40,f=0.01,rho=2,lambda0=5,phi0=0.00385, m = 52000, m1=71)
coverage10_fdr1 <- est_power_curve(n=40,f=0.01,rho=2,lambda0=10,phi0=0.00385)
coverage5_fdr5 <- est_power_curve(n=40,f=0.05,rho=2,lambda0=5,phi=0.00385)
coverage10_fdr5 <- est_power_curve(n=40,f=0.05,rho=2,lambda0=292,phi=0.00385, m = 52000, m1=71)
coverage20_fdr5 <- est_power_curve(n=40,f=0.05,rho=2,lambda0=20,phi=0.00385)
plot_power_curve(list(coverage5_fdr1,coverage5_fdr5,coverage10_fdr1,coverage10_fdr5,coverage20_fdr5))
```

### est_power_curve

![](https://i.imgur.com/sYRvKqB.png)

```{r, optimize, tidy=TRUE, echo=FALSE, fig.cap="Blue to brown gradient shows power from 0 to 1. Here FDR=0.01."}
# optimization plots
result <- optimize_parameter(fun=est_power, opt1="n", opt2="lambda0", opt1Value=c(5,10,15,20),opt2Value = c(5,10,15,20), f=0.01, phi0=0.00385)
```
